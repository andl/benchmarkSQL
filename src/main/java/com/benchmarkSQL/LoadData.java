...
public class LoadData implements jTPCCConfig {
	...
	private static String configInfo = "N/A";
	 ...
	 *            <li>-h: Whether this is a 'hot start run', i.e. the first time this database process has done anything. This is recorded,
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			} else if (str.toLowerCase().startsWith("-c")) {// any extra config information.
				configInfo = args[i].substring("-c".length());
			} else if (str.toLowerCase().startsWith("-h")) { // hot start
				hotStart = args[i].substring("-h".length());
			} else if (str.toLowerCase().startsWith("-s")) {// silent?
				silent = Boolean.valueOf(args[i].substring("-h".length())).booleanValue();
    ...
    int custPerDist = factorize(configCustPerDist, scale);
		...
		if (!silent) {
			...
			System.out.println("Rows Per Second = " + rowsLoadedPerSecond + " Rows/Sec");
			System.out.println("------------------------------------------------------");
		}

		writeResultToResultsFile(logFileLocation, name, runTime, totalRows, rowsLoadedPerSecond);
	...
	private static void writeResultToResultsFile(String logFileLocation, String executingDatabaseName, long runTime, long rowsLoaded,
			long rowsLoadedPerSecond) {
		String resultsFolderPath = logFileLocation + "benchmark-results" + File.separator + "load-data-summary";

		// Create folder if it doesn't exist.
		File resultsFolder = new File(resultsFolderPath);
		resultsFolder.mkdirs();

		// Add results to a CSV file for a single database, and to a generic file for all database results.
		File specificDbResultsFile = new File(resultsFolderPath + File.separator + executingDatabaseName + ".csv");
		addToResultsFile(executingDatabaseName, runTime, rowsLoaded, rowsLoadedPerSecond, specificDbResultsFile);

		File genericResultsFile = new File(resultsFolderPath + File.separator + "all.csv");
		addToResultsFile(executingDatabaseName, runTime, rowsLoaded, rowsLoadedPerSecond, genericResultsFile);
	}

	public static void addToResultsFile(String executingDatabaseName, long runTime, long rowsLoaded, long rowsLoadedPerSecond,
			File resultsFile) {
		...
		try {
			...
			if (resultsFileCreated) {
				...
				String header = "timeOfTest, runTime, databaseBeingTested, rowsLoaded, rowsLoadedPerSecond, hotStart, config, replicas" + "\n";

				resultsFileStream.write(header.getBytes());
			}

			String results = startDate.getTime() + ", " + runTime + ", " + executingDatabaseName + ", " + rowsLoaded + ", "
					+ rowsLoadedPerSecond + ", " + hotStart + ", " + configInfo + ", " + numberOfReplicas + "\n";
	...
	static void initJDBC() {
		...
		try {
			...
			distPrepStmt = conn.prepareStatement("INSERT INTO district "
					+ " (d_id, d_w_id, d_ytd, d_tax, d_next_o_id, d_name, d_street_1, d_street_2, d_city, d_state, d_zip) "
					+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

			itemPrepStmt = conn.prepareStatement("INSERT INTO item " + " (i_id, i_name, i_price, i_data, i_im_id) "
					+ "VALUES (?, ?, ?, ?, ?)");

			custPrepStmt = conn.prepareStatement("INSERT INTO customer " + " (c_id, c_d_id, c_w_id, "
					+ "c_discount, c_credit, c_last, c_first, c_credit_lim, " + "c_balance, c_ytd_payment, c_payment_cnt, c_delivery_cnt, "
					+ "c_street_1, c_street_2, c_city, c_state, c_zip, " + "c_phone, c_since, c_middle, c_data) "
					+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

			histPrepStmt = conn.prepareStatement("INSERT INTO history " + " (h_c_id, h_c_d_id, h_c_w_id, " + "h_d_id, h_w_id, "
					+ "h_date, h_amount, h_data) " + "VALUES (?, ?, ?, ?, ?, ?, ?, ?)");

			ordrPrepStmt = conn.prepareStatement("INSERT INTO oorder " + " (o_id, o_w_id,  o_d_id, o_c_id, "
					+ "o_carrier_id, o_ol_cnt, o_all_local, o_entry_d) " + "VALUES (?, ?, ?, ?, ?, ?, ?, ?)");

			orlnPrepStmt = conn.prepareStatement("INSERT INTO order_line " + " (ol_w_id, ol_d_id, ol_o_id, "
					+ "ol_number, ol_i_id, ol_delivery_d, " + "ol_amount, ol_supply_w_id, ol_quantity, ol_dist_info) "
					+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

			nworPrepStmt = conn.prepareStatement("INSERT INTO new_order " + " (no_w_id, no_d_id, no_o_id) " + "VALUES (?, ?, ?)");

			stckPrepStmt = conn.prepareStatement("INSERT INTO stock "
			...
			whsePrepStmt = conn.prepareStatement("INSERT INTO warehouse "
